...
def test_zero_shot_infer():
    """
    Runs a zero-shot classification test on a single randomly selected image
...
def test_zero_shot_eval():
    """
    Evaluates the zero-shot classifier on all JPG images in the validation set.
    ...
    # Hard assert minimum baseline
    assert acc > 0.4, "Model performance is too low!!!"


if __name__ == "__main__":
    # test_zero_shot_infer()
    test_zero_shot_eval()